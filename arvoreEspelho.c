...
int eh_espelho(Arvore *arv_a, Arvore *arv_b) {
    return (arv_a == NULL && arv_b == NULL)
            || ((arv_a != NULL && arv_b != NULL)
                && (arv_a->info == arv_b->info
...
Arvore *cria_espelho(Arvore *arv_a) {
    ...
    return arv_a == NULL ? cria_arv_vazia() : arv_constroi(arv_a->info, cria_espelho(arv_a->dir), cria_espelho(arv_a->esq));
}
